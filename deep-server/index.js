          ...
          async.waterfall(
            [
              ...
              function(callback) {
                ...
                unirest
                  .post("https://openapi.naver.com/v1/language/translate")
                  .headers({
              ...
              function(tID, callback) {
                console.log("3.find target id == ", tID);
    ...
    async.waterfall(
          ...
          db.Chats.findOne(
            ...
            (err, result) => {
              ...
              if (result) {
                ...
                if (country == "ko") {
                  var tmp = [];
                  console.log("user country is ko ==> start to translation");
                  forfunction = function(i, callback) {
                    unirest
                      .post("https://openapi.naver.com/v1/language/translate")
                      .headers({
                        "Content-Type": "application/x-www-form-urlencoded",
                      ...
                      .end(function(res) {
                        ...
                        // console.log(null, res.body.message.result.translatedText);
                        //console.log(i + ": " + res.body.message.result.translatedText)
                        tmp.push(
                          result.des[i].split(":")[0] +
                            ": " +
                            res.body.message.result.translatedText
                        );
                        if (i >= result.des.length - 1) {
                          callback();
                        } else {
                          forfunction(i + 1, callback);
                        }
                      });
                  };
                  forfunction(0, function() {
                    console.log(tmp);
                    callback(null, tmp);
                  });
                } else if (country == "en") {
                  callback(null, result.des);
                }
              }
            }
          );
      ...
      function(err, result) {
        if (err) console.log("err : " + err);
          ...
          socket.emit("loading message", { des: result });
        }
      }
    );
  });
